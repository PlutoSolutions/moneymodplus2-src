...
public class HoleFiller extends Module {
	private final Setting range = new Setting.Builder(SettingType.INTEGER).setName("Range").setModule(this)
			.setIntegerValue(3).setMinIntegerValue(1).setMaxIntegerValue(5).build();

	private final Setting radius = new Setting.Builder(SettingType.INTEGER).setName("Radius").setModule(this)
			.setIntegerValue(5).setMinIntegerValue(1).setMaxIntegerValue(10).build();

	public HoleFiller(String name, String description, Category category) {
		...
		for (final EntityPlayer entityPlayer : mc.world.playerEntities) {
			if (!entityPlayer.getUniqueID().equals(mc.player.getUniqueID())) {
				...
				for (final BlockPos blockPos : BlockPos.getAllInBox(
						...
						position.add(doubleValue, doubleValue, doubleValue))) {
					if (mc.player.getDistanceSqToCenter(blockPos) > this.range.getIntegerValue()) {
						continue;
					}
